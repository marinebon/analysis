...
rvc_spp = read_csv(rvc_spp_csv) %>%
  select(
    year, dataset_id = datasetID, 
    mapgridnum = mapGridNumber, scientific_name = scientificName, q_mean)
...
rvc_tbl = rvc_tbl %>%
  mutate(
    specnum  = map(data_wide, function(x) select(x, -dataset_id, -mapgridnum) %>% specnumber))
    specnum  = map(data_wide, function(x) select(x, -dataset_id, -mapgridnum) %>% specnumber))

rarefy(rvc_tbl$data_wide[[1]] %>% select(-dataset_id, -mapgridnum), se = F, Margin = 1)
