...
1. Differentiate 4 sections: 
  - upper/lower: more protected by land
  - middle: nutrient & freshwater influx from Bay
...
1. By Strata name
    1. Forereef Deep Linear Reef 
    1. Forereef Midchannel Linear Reef
    1. Forereef Shallow Linear Reef
    1. High Relief Reef
    1. Inshore Patch Reef 
    1. Midchannel Patch Reef 
...
rvc_mg  = read_csv('data/rvc_mg.csv')
...
bscols(
  leaflet(sd) %>% addTiles() %>% addMarkers(lat=~lat, lng=~lon),
...
rvc_spp_missing = unique(rvc_spp$scientificName)[!unique(rvc_spp$scientificName) %in% unique(spp_traits$Latin_name)]
# 30 of 362 rvc_spp$scientificName not in spp_traits$Latin_name
...
spp_traits_missing = unique(spp_traits$Latin_name)[!unique(spp_traits$Latin_name) %in% unique(rvc_spp$scientificName)]
...
rvc_spp %>%
  #filter(scientificName=='Panulirus argus')   # lobster (n=0)
  filter(scientificName=='Diadema antillarum') # urchin (n=0)
...
rvc_tbl = rvc_spp %>%
  nest(-year) %>%
  mutate(
    data_wide = map(data, ~ spread(data=.x, scientificName, q_mean, fill=0)))
```
...
rvc_tbl = rvc_tbl %>%
  mutate(
    norm = map(data_wide, ~ vegdist(
      data = .x %>% select(-datasetID, -mapGridNumber), 
      method="bray", binary=F, diag=F, upper=F)))
 
#norm = vegdist(rvc_tbl$data_wide[[1]] %>% select(-datasetID, -mapGridNumber),  method="bray", binary=F, diag=F, upper=F)                  
...
```{r Richness}  
  ...
  richness = rarefy(x, sample, se = F, Margin = 1)
```  
- **Evenness**: 
...
```{r Pielou's Evenness}
J <- diversity(x, index = 'shannon')/log(specnumber(x)) #where x is abundance dataframe 
```
Spatial variants:
- **alpha**
- **gamma**
