...
```{r Effective Simpson}
# test with one
# x = rvc_tbl$data_wide[[1]] # mapgridnum x species
# eff_simpson = 1 / (1 - diversity(x %>% select(-dataset_id, -mapgridnum, -n_stations), index='simpson'))

rvc_tbl = rvc_tbl %>%
  mutate(
    eff_simpson = map(data_wide, function(x) 1 / (1 - diversity(x %>% select(-dataset_id, -mapgridnum, -n_stations), index='simpson'))))
...
rvc_tbl = rvc_tbl %>%
  mutate(
    Pielous_evenness = map(data_wide, function(x) diversity(x %>% select(-dataset_id, -mapgridnum, -n_stations), index='shannon')/log(specnumber(x))))

hist(rvc_tbl$Pielous_evenness[[1]])
...
# TODO: dataset_id + mapgridnum -> grid_id
...
func.div=1/(1-apply(w_mcq,1,function(x) t(x) %*% as.matrix(func.dist) %*% x)) #Error in t(x) %*% as.matrix(func.dist) : requires numeric/complex matrix/vector arguments
...
#summarize by BASIN, SEGMENT, DATE, LATDEC, LONDEC, SITE, STA, TEMP-S, TEMP-B
temp = WQ %>% 
    select(BASIN, SEGMENT, DATE, LATDEC, LONDEC, SITE, STATION, surftemp = `TEMP-S`, bottemp = `TEMP-B`) %>% 
    tolower() %>%
    group_by(date, latdec, londec, surftemp, bottemp) #error object class 'character' 
    View(temp)
