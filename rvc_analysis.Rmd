...
```{r count_groups_by_year}
rvc_spp = read_csv('data/rvc_spp_grouped.csv') # View(rvc_spp)
...
# convert to points
pts_rich_gcs = spp_rich
coordinates(pts_rich_gcs) = ~ x + y
proj4string(pts_rich_gcs) = prj_gcs
p = spTransform(pts_rich_gcs, CRS(prj_mer)) # %>% st_as_sf()

# TODO: offset c(0.5,0.5) to get all points?
h = spsample(pts_rich_mer,  type="hexagonal", cellsize=10000, offset=c(0,0)) %>%
...
leaflet(h_gcs) %>%
  ...
  addPolygons(
    ...
    color = ~colorQuantile('Spectral', h_p$n_spp)(n_spp))
# yay! works
...
# Create a palette that maps factor levels to colors
pal <- colorNumeric('YlOrRd', spp_rich$n_spp, na.color = "transparent") # 'Spectral'
...
leaflet(spp_rich) %>% 
  ...
  addLegend("topleft", pal = pal, values = spp_rich$n_spp,
    title = "n spp", opacity = 1)
```
