...
# Prep data for domain biodiversity metrics 
...
mg_cord_q <- left_join(rvc_mg, rvc_spp) #added lat, long: datasetID, mapGridNumber, latitude, longitude, year, protection
...
mg_cord_tbl = mg_cord_q %>% #year, subregion, data(list)
...
# Prep data for subregion biodiversity metrics
```{r subregion data wide}
...
ggplot(data = mg_cord_tbl, aes(x = year, y = eff_simpson_avg_mg, subregion, group = subregion)) +
...
#simpson and shannon 
ggplot(data = mg_cord_tbl, aes(x = year)) +
...
```{r prep data for level of protection}

# mg_protection = mg_cord_q %>% #year, subregion, data(list)
#   group_by(year, protection)  %>%
#   nest() #year, subregion, protection, data(list of 8 variables: datasetID mapGridNumber latitude longitude protection scientificName q_mean n_stations)
# 
# x = mean()
# apply(x %>% select(-c(1:6)), 2, mean)
# 
# mg_protection = mg_protection %>%
#   mutate(
#     sp_abun_prot = map( #datasetID,mapGridNumber, lat, long, protection, n_stations, all species detected 
#       data, #datasetID,mapGridNumber, lat, long, protection, scientificName, q_mean, n_stations
#       ~ spread(data=.x, scientificName, q_mean, fill=0))) 
# 
# mg_protection =  mg_protection %>%
#   mutate(
#     effective_simpson_prot = map( #effective simpson by mapgrid 
#       sp_abun_prot, 
#       function(x) 1 / (1 - diversity(x %>% select(-c(1:6)), index='simpson'))),
#     eff_simpson_avg_prot = map_dbl(
#       sp_abun_prot, 
#       function(x){
#         x = apply(x %>% select(-c(1:6)), 2, mean)
#         1 / (1 - diversity(x, index='simpson'))}))
...
# # TODO: Why/how are eff_simpson_avg_mg & eff_simpson_avg_mg different? Which to use?
# 
# ggplot(data = mg_protection, mapping = aes(x = year, y = eff_simpson_avg_prot, color = protection, group = protection)) + 
#   geom_point() + 
#   geom_line() +
#   facet_wrap(~ protection, nrow = 4) +
#   ggtitle("Simpson Diversity in the FKNMS") +
#   labs(x = "Year", y = "Effective Number of Species", colour = "Protection") +
#   scale_color_hue(labels = c("Protected", "Kinda Protected", "Not Protected"))
# 
# qplot(year, eff_simpson_avg_prot, data = mg_protection, color = subregion, geom = c("point", "smooth"), method = "loess", se = T) +
#   facet_grid(subregion ~ .) +
#   theme(strip.background = element_blank(), strip.text.y = element_blank()) +
#   ggtitle("Simpson Diversity") +
#   labs(x = "Year", y = "Effective Number of Species", colour = "Subregion") +
...
mg = mg_cord_tbl %>% unnest(sp_abun_sr, sp_richness_sr, effective_simpson_sr, effective_shannon_sr) #year, subregion, eff_simpson_avg_mg, eff_shannon_avg_mg, sp_richness_sr, effective_simpson_sr, effective_shannon_sr, datasetID, mapGridNumber, latitude, longitude, protection, n_stations, and all species
