...
rvc_spp = read_csv(rvc_spp_csv) %>%
  select(
    ...
    q_mean)
  left_join(subregion, by('mapgridnum')) #add column 'subregion' to rvc_spp
  group_by(year, subregion, scientificName) #group by year, subregion, and scientific name 
  spread()


abundances = #community by species matrix (year and subregion by species with abundance values)
