...
```{r simpson data wide with lat long}

#Want data in long format with columns: year, subregion, mapgridnumber, lat, long, strata, protection, richness, simpson, shannon 

taxa2 <- separate(data = taxa2, col = taxa2, into = c('kingdom','phylum','class','order','family','genus','species'), sep = ",")

simpson_wide <- str_replace_all(mg_cord_tbl$effective_simpson_sr, "[c]", "=") 
  
trial <- separate(data = mg_cord_tbl, col = sp_abun_sr, into = c('datasetID', 'mapGridNumber', 'latitude', 'longitude', 'protection' ), sep = ",")
