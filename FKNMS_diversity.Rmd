...
```{r Effective Functional diversity for FKNMS domain}
...
qplot(year, eff_simpson_avg_mg, data = subregion_tbl, color = subregion, geom = c("point", "smooth"), method = "loess", se = T) +
  facet_grid(subregion ~ .) +
  theme(strip.background = element_blank(), strip.text.y = element_blank()) +
  ggtitle("Simpson Diversity") +
...
mpa_tbl =  mpa_tbl %>%
  mutate(
    ...
    eff_shannon_avg_mpa = map_dbl(
      ...
      function(x){
        x = apply(x %>% select(-c(1:6)), 2, mean)
        exp(diversity(x %>% select(-c(1:6)), index='shannon'))}))
