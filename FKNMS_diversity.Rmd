...
upper_keys <- filter(rvc_mg, 
                    latitude > 24.95)
middle_keys <- filter(rvc_mg, 
                    latitude >24.63 & latitude <= 24.95, 
                    longitude >= -81.10 & longitude <= -80.45)
lower_keys <- filter(rvc_mg, 
                    latitude >24.55 & latitude <= 24.63, 
                    longitude >= -82.65 & longitude <= -81.10)
dry_tortugas <- filter(rvc_mg, 
                    latitude >24.55 & latitude <= 24.75, 
                    longitude >= -83.5 & longitude <= -82.65)

left_join() #add column 'subregions' with observations: UK, MK, LK, DT
group_by() #year, subregion, scientificname
effective_simpson = map(
      data_wide, 
      function(x) 1 / (1 - diversity(x, %>% (select(), index='simpson')))))# calculate effective simpson diversity for each year and subregion 
  
ggplot(data = x, mapping = aes(x = year, y = effective_simpson)) + 
